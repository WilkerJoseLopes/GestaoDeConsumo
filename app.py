...
def home():
    ...
    # Prepara dados para JS: extrai lat, lng e certificado energético
    casas = []
    for item in dados:
        try:
            lat = float(item.get('lat', 0))
            lng = float(item.get('lng', 0))
            cert = item.get('certificadoenergetico', '').strip().upper()
            casas.append({'lat': lat, 'lng': lng, 'cert': cert})
        except:
            pass

    # Renderiza o template HTML com dados embutidos no JS
...
/>
<style>
    /* Seu CSS aqui (igual ao que você já tinha) */
    body {
        margin: 0;
...
<footer>
    ...
    <div id="footer-direita">Sobre o projeto</div>
</footer>

<script src="https://unpkg.com/leaflet@1.9.4/dist/leaflet.js"></script>
<script>
document.addEventListener("DOMContentLoaded", function () {
    const dadosCasas = {{ casas|tojson }};
