...
.modal-content {background:white; padding:30px; border-radius:8px; text-align:center; box-shadow:0 0 15px rgba(0,0,0,0.3);}
...
<header>
  ...
  <div id=\"header-right\">
    ...
    {% if not session.get('logado') %}<span onclick=\"abrirModal()\">Área Privada</span>{% else %}<span onclick=\"confirmarLogout()\">Logout</span>{% endif %}
  </div>
</header>
<main>
  {% if mensagem %}<div class=\"alert\">{{ mensagem }}</div>{% endif %}
  <div id=\"form-coords\">
...
function abrirModal(){ document.getElementById('loginModal').classList.add('show'); }
function fecharModal(){ document.getElementById('loginModal').classList.remove('show'); }

const map = L.map('map').setView([41.1578, -8.6291], 12);
L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png').addTo(map);
const cores = { 'A+':'008000','A':'00AA00','A-':'33BB33','B+':'66CC00','B':'99CC00','B-':'BBD600','C+':'CCCC00','C':'FFFF00','C-':'FFDD00','D+':'FFB300','D':'FFA500','D-':'FF8800','E+':'FF6666','E':'FF0000','E-':'CC0000','F+':'A00000','F':'8B0000','F-':'660000','G+':'444444','G':'000000','G-':'222222','':'0000FF'};
function criarIcone(c){
  const svg = `<svg xmlns=\"http://www.w3.org/2000/svg\" width=\"32\" height=\"45\" viewBox=\"0 0 32 45\"><path fill=\"#${c}\" stroke=\"black\" stroke-width=\"2\" d=\"M16,1 C24.3,1 31,7.7 31,16 C31,27 16,44 16,44 C16,44 1,27 1,16 C1,7.7 7.7,1 16,1 Z\"/></svg>`;
...
fetch('/todas_casas').then(r=>r.json()).then(casas => {
  casas.forEach(c => {
    ...
    let texto = `<strong>${c.morada}</strong><br>${c.descricao}<br>Latitude: ${c.latitude.toFixed(5)}<br>Longitude: ${c.longitude.toFixed(5)}<br>Certificado: <strong>${c.certificado}</strong>`;
